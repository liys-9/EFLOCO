    ...
    def sample(
        ...
        with ctx:
            for i in range(n_steps):
                ...
                #
                # w = 1.5
                # # # # # p_1t = (1 + w) * p_1t - w * bad_pred
                # # # #
                # # # # # print('p_1t',p_1t,p_1t.shape)
                # #
                # # #
                # bad_pred = torch.softmax(bad_pred, dim=1)
                # # #
                # p_1t = (1 + w) * p_1t - w * bad_pred
                # p_1t = p_1t.clamp(min=0.0)
                # # p_1t = p_1t - p_1t.amin(dim=(1), keepdim=True).clamp(max=0)
                #
                # # print('p_1t_softmax', p_1t, p_1t.shape)
                if not self.sparse:
